...
int doPipe(char **args, int pipei)
{
  ...
  else
  {
    ...
    // basically a mini main
    int start = 0;
    while(start < rightCount)
    {
      int end;
      bool waitfor = parse(rightTokens, start, &end);
      if(end >= start) doCommand(rightTokens, start, end, waitfor);
      start = end + 1;
      while(rightTokens[start] && (equal(rightTokens[start], "&") || equal(rightTokens[start], ";"))) start++;
    }
    // Free right tokens-------------------------------
...
char* subsituteBackticks(char* token) {
  ...
  char* command = malloc(tokenLen - 1);

  int commandLen = tokenLen - 2; // the length of the token with backticks removed
  ...
  if (pid == 0) { // CHILD PROCESS (will execute command)
    ...
    char buf[256];
    int resultLen = read(pipeFD[READ], buf, 256);
    buf[resultLen] = '\0';
  ...
  while (should_run)
  {
    ...
    for (int i = 0; args[i] != NULL; i++) {
      if (args[i][0] == '`' && args[i][strlen(args[i]) - 1] == '`') {
        ...
        char *executedToken = subsituteBackticks(args[i]);

        // replace old token 
        free(args[i]);
        args[i] = executedToken;
      }
    }
